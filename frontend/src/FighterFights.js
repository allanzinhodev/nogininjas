...
const FighterFights = ({ fighterId }) => {
  ...
  useEffect(() => {
    const fetchFights = async () => {
      try {
        // Chama o endpoint fighterfights passando o fighterId
        const response = await axios.get(`/fighterfights?id=${fighterId}`);
        setFights(response.data); // Define os dados recebidos no estado
        setLoading(false);
      } catch (err) {
